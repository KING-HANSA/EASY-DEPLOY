...
cmd({
  ...
  try {
    ...
    if (!fs.existsSync('./.git')) {
      ...
      execSync('git init');
      execSync('git remote add origin https://github.com/KING-HANSA/DEW-MD-BETA-TEST.git');
    } else {
    ...
    if (localCommit === originCommit) {
      await conn.sendMessage(from, { text: '*✅ DEW-MD is already up to date!*' }, { quoted: mek });
    } else {
      ...
      await conn.sendMessage(from, { text: '*✅ DEW-MD updated successfully!*' }, { quoted: mek });
    }
  } catch (error) {
    console.error(error);
